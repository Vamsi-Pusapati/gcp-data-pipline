...
### 📐 High-Level End-to-End Architecture (Summary)
```
(1) Extract      (2) Event Trigger     (3) Ingest / Load          (4) Enrich              (5) Serve / Query            (6) AI / RAG Augment
┌────────────┐   ┌────────────────┐    ┌──────────────────┐      ┌────────────────┐     ┌────────────────────┐      ┌────────────────────────┐
│Medicaid API│→→ │Composer DAG    │ →→ │GCS Raw (JSON)     │ →→   │Cloud Run Func  │ →→  │BigQuery Staging     │ →→   │Dataproc PySpark Job     │
└────────────┘   │medicaid_data_dag│   │Pub/Sub ObjectEvt │      └────────────────┘     └────────────────────┘       └────────────────────────┘
                                        │ (Finalize)                │  Validate / Load          │ Read / Transform          │ Enrich (parse, map)
                                        ▼                          ▼                          ▼                          ▼
                                   ┌────────────────┐        ┌──────────────────┐      ┌────────────────────┐      ┌────────────────────┐
                                   │GCS Processed    │        │BigQuery Staging  │      │Dataproc Output Temp│      │BigQuery Enriched   │
                                   └────────────────┘        └──────────────────┘      └────────────────────┘      └────────────────────┘
                                                                                                    │                            │
                                                                                                    │ (Queries + RAG Context)    │ (Embeddings Build)
                                                                                                    ▼                            ▼
                                                                                  ┌────────────────────────────────────┐   ┌──────────────────────────────┐
                                                                                  │Streamlit Dashboard (GKE + AR Img)│   │Vertex AI RAG (Emb + FAISS + LLM)│
                                                                                  │- Charts / Filters / Metrics      │   │- text-embedding-gecko          │
                                                                                  │- Live BigQuery Queries          │   │- FAISS similarity index        │
                                                                                  │- Chatbot (Gemini Pro)           │   │- Gemini Pro responses          │
                                                                                  └────────────────┬──────────────────┘   └──────────────────────────────┘
                                                                                                   │
                                                                                                   ▼
                                                                                             ┌──────────────┐
...
Vertex AI RAG Flow:
BigQuery Enriched → Embeddings (text-embedding-gecko) → FAISS Index → User Query Embedding → Top-K Context → Gemini Pro Answer

Legend:
- ObjectEvt: GCS Object Finalize Pub/Sub event
- AR Img: Artifact Registry container image
- RAG: Retrieval-Augmented Generation (Embeddings + Vector Index + LLM)
```
